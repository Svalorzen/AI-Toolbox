#ifndef AI_TOOLBOX_MDP_PRIORITIZED_SWEEPING_GENERAL_HEADER_FILE
#define AI_TOOLBOX_MDP_PRIORITIZED_SWEEPING_GENERAL_HEADER_FILE
...
namespace AIToolbox {
    namespace MDP {
        ...
        class PrioritizedSweepingGeneral;
#endif
         ...
         */
        template <typename M>
        class PrioritizedSweepingGeneral<M> {
            public:
                /**
                 ...
                 */
                PrioritizedSweepingGeneral(const M & m, double theta = 0.5, unsigned n = 50);

                /**
                 * @brief This function updates the PrioritizedSweepingGeneral internal update queue.
                 *
        ...
        template <typename M>
        bool PrioritizedSweepingGeneral<M>::PriorityTupleLess::operator() (const PriorityQueueElement& arg1, const PriorityQueueElement& arg2) const
        {
        ...
        template <typename M>
        void PrioritizedSweepingGeneral<M>::batchUpdateQ() {
        ...
        template <typename M>
        unsigned PrioritizedSweepingGeneral<M>::getN() const {
            return N;
        }
        ...
        template <typename M>
        double PrioritizedSweepingGeneral<M>::getQueueThreshold() const {
            return theta_;
        }

        template <typename M>
        size_t PrioritizedSweepingGeneral<M>::getQueueLength() const {
            return queue_.size();
        }

        template <typename M>
        const M & PrioritizedSweepingGeneral<M>::getModel() const {
            return model_;
        }

        template <typename M>
        const QFunction & PrioritizedSweepingGeneral<M>::getQFunction() const {
            return qfun_;
        }

        template <typename M>
        void PrioritizedSweepingGeneral<M>::setQFunction(const QFunction & qfun) {
            qfun_ = qfun;
        }

        template <typename M>
        const ValueFunction & PrioritizedSweepingGeneral<M>::getValueFunction() const {
            return vfun_;
        }
    }
}
