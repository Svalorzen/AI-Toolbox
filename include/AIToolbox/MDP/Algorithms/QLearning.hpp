...
namespace AIToolbox {
    namespace MDP {
         ...
         * QFunction for a generative MDP model, where the transition and
         * reward functions are unknown. It works in an offline fashion,
         * meaning that it can be used even if the policy that the agent is
         * currently using is not the optimal one, or is different by the one
         * currently specified by the QLearning QFunction.
         *
         ...
         * given that the learning parameter is correctly set.
         *
         ...
         * world's behavior aside from actually try out actions.
         */
        template <typename M>
        class QLearning<M> {
            public:
