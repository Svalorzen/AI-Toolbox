#define BOOST_TEST_MODULE Factored_Bandit_FlattenedModel
...
BOOST_AUTO_TEST_CASE( construction ) {
    ...
    fb::Model<std::normal_distribution<double>> bandit(std::move(A), std::move(groups), {std::move(testa), std::move(testb)});

    fb::FlattenedModel flat(bandit);

    BOOST_CHECK_EQUAL(flat.getA(), 8);
}
