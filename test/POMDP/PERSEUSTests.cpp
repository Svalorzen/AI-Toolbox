#define BOOST_TEST_MODULE POMDP_PERSEUS
...
BOOST_AUTO_TEST_CASE( discountedHorizon ) {
    ...
    // For higher horizons PERSEUS may not find all the possible solutions, but
    // generally gets close. The solution also depends on which beliefs were
    // randomly sampled.
    unsigned horizon = 5;
    POMDP::PERSEUS solver(1000, horizon);
    ...
    for ( size_t i = 0; i < vf.size(); ++i ) {
        ...
        if ( !sizeEqual2 ) {
            std::cout << "truth: " << vt[i].size() << " - ours: " << vf[i].size() << "\n";
            std::cout << "i : " << i << "\n";
            continue;
        }
...
BOOST_AUTO_TEST_CASE( undiscountedHorizon ) {
    ...
    // We check that all entries PERSEUS found exist in the ground truth.
