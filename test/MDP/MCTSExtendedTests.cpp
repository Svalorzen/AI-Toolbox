...
BOOST_AUTO_TEST_CASE( escapeToCorners ) {
    ...
    auto get = [](const std::tuple<size_t, boost::dynamic_bitset<>> &t) { return std::get<0>(t); };
    ...
    BOOST_CHECK_EQUAL( get(solver.sampleAction(1,10)), LEFT);
    BOOST_CHECK_EQUAL( get(solver.sampleAction(2,10)), LEFT);

    // Last cell of first row wants to either go down or left
    auto a = get(solver.sampleAction(3,10));
    BOOST_CHECK( a == LEFT || a == DOWN );

    // Middle cells of first column want to go up
    BOOST_CHECK_EQUAL( get(solver.sampleAction(4,10)), UP);
    BOOST_CHECK_EQUAL( get(solver.sampleAction(8,10)), UP);

    // Cell in 1,1 wants left + up
    a = get(solver.sampleAction(5,10));
    BOOST_CHECK( a == LEFT || a == UP );

    // Middle cells of last column want to go down
    BOOST_CHECK_EQUAL( get(solver.sampleAction(7, 10)), DOWN);
    BOOST_CHECK_EQUAL( get(solver.sampleAction(11,10)), DOWN);

    // Cell in 2,2 wants right + down
    a = get(solver.sampleAction(10,10));
    BOOST_CHECK( a == RIGHT || a == DOWN );

    // Bottom cell in first column wants up + right
    a = get(solver.sampleAction(12,10));
    BOOST_CHECK( a == RIGHT || a == UP );

    // Finally, bottom middle cells want just right
    BOOST_CHECK_EQUAL( get(solver.sampleAction(13,10)), RIGHT);
    BOOST_CHECK_EQUAL( get(solver.sampleAction(14,10)), RIGHT);
}
