...
namespace AIToolbox {
    Model::Model(size_t s, size_t a) : S(s), A(a), experience_(s,a), transitions_(boost::extents[S][S][A]), rewards_(boost::extents[S][S][A]),
                                       rand_(std::chrono::system_clock::now().time_since_epoch().count()), sampleDistribution_(0.0, 1.0)
    {
        update();
    }

    Model::Model(Experience exp) : S(exp.getS()), A(exp.getA()), experience_(exp), transitions_(boost::extents[S][S][A]), rewards_(boost::extents[S][S][A]) {
        update();
    }
    ...
    void Model::update(size_t s, size_t a) {
    ...
    std::tuple<size_t, double> Model::sample(size_t s, size_t a) const {
    ...
    Experience &       Model::getExperience()       { return experience_; }
    const Experience & Model::getExperience() const { return experience_; }
