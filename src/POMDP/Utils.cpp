...
namespace AIToolbox {
    namespace POMDP {
        VList crossSum(const VList & l1, const VList & l2) {
            VList c;

            if ( ! ( l1.size() && l2.size() ) ) return c;

            size_t S = l1[0].second.size();
            size_t a = l1[0].first;
        ...
        VList::iterator findBestVector(const Belief & belief, VList::iterator begin, VList::iterator end) {
            ...
            double bestValue = dotProd(belief, bestMatch->second);

            while ( (++begin) < end ) {
                double currValue = dotProd(belief, begin->second);
